...
def filter_low_pass(img_arr, b=2, kernel_size=3, pad_method="edge"):
    M, N = img_arr.shape
    result = np.zeros( (M, N) )
    n_pad = kernel_size // 2
    ...
    for i in range(M):
        for j in range(N):
            result_part = img_expand_arr[i: i + kernel_size, j: j + kernel_size] * mask
...
# Find best params
# Remember comment it before submit hw1
'''
...
# uniform noise, low-pass filter
result8_1_arr = filter_low_pass(sample6_arr, b=2, kernel_size=5)
utils.save_npArr2JPG(result8_1_arr, "8_result")
# impulse noise, medial filter
result9_2_arr = filter_median(sample7_arr, kernel_size=3, percent=50)
result9_3_arr = filter_median(result9_2_arr, kernel_size=3, percent=50)
utils.save_npArr2JPG(result9_2_arr, "tmp/9_result_once")
utils.save_npArr2JPG(result9_3_arr, "9_result")
...
utils.save_npArr2JPG(result8_2_arr, "tmp/8_result_low_pass2")
result8_3_arr = filter_median(sample6_arr, kernel_size=3)
utils.save_npArr2JPG(result8_2_arr, "tmp/8_result_median")
result9_1_arr = filter_low_pass(sample7_arr, b=1, kernel_size=5)
utils.save_npArr2JPG(result9_1_arr, "tmp/9_result_low_pass")
result8_4_arr = filter_fft(sample6_arr, "tmp/sample6")
utils.save_npArr2JPG(result8_3_arr, "tmp/8_result_fft")
result9_4_arr = filter_fft(sample7_arr, "tmp/sample7")
utils.save_npArr2JPG(result9_3_arr, "tmp/9_result_fft")
result9_5_arr = filter_median(result9_3_arr, kernel_size=3, percent=50)
'''
...
print('PSNR of uniform noise by low-pass : {0:.4f}'.format(PSNR(sample5_arr, result8_1_arr)))
print('PSNR of impulse noise by median_1: {0:.4f}'.format(PSNR(sample5_arr, result9_2_arr)))
print('PSNR of impulse noise by median_2: {0:.4f}'.format(PSNR(sample5_arr, result9_3_arr)))

# Remember comment it before submit hw1
'''
print('PSNR of uniform noise by low-pass b=3 : {0}'.format(PSNR(sample5_arr, result8_2_arr)))
print('PSNR of uniform noise by median : {0}'.format(PSNR(sample5_arr, result8_3_arr)))
print('PSNR of uniform noise by fft: {0:.4f}'.format(PSNR(sample5_arr, result8_4_arr)))
print('PSNR of impulse noise by low-pass: {0}'.format(PSNR(sample5_arr, result9_1_arr)))
print('PSNR of impulse noise by fft: {0:.4f}'.format(PSNR(sample5_arr, result9_4_arr)))
print('PSNR of impulse noise by median_3: {0:.4f}'.format(PSNR(sample5_arr, result9_5_arr)))
'''
